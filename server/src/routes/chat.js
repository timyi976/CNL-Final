...
router.post("/new", async (req, res) => {
    ...
    if (user_puid.length !== 0) {res.status(409).json({error: "uid exists"}); return}

    //check if uid is already in db 
    let user_uid = await User.find({uid: req.body.uid})
    if (user_uid.length !== 0) {res.status(409).json({error: "puid existd"}); return}
...
router.post("/chat", async (req, res) => {
    ...
    if (!target_user) {res.status(404).json({error: "target user not found"}); return}
    ...
    if (chatbox.length !== 0) {res.status(409).json({error: "chatroom has created"}); return}

    //check if the target platform bot work correctly
    await axios.post(url[target_user.plat] + '/create', { puid: target_user.puid })
    .then( async (bot_res) =>  {
    ...
    ).catch((err) => {res.status(404).json({error: "target platform bot error"}); return});
} );

router.post("/send", async (req, res) => {
    //find the chatbox by pgid
    let chatbox = await Chatbox.findOne({$or: [{pgid1: req.body.gid}, {pgid2: req.body.gid}]})
    if (!chatbox) {res.status(404).json({error: "chatroom not found"}); return}
    ...
    await axios.post(url[target_plat] + '/send', { gid: target_pgid, message: req.body.message })
